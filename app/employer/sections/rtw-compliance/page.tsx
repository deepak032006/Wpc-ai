    ...
    <div style={{ backgroundColor: "white", borderBottom: "1px solid #E2E8F0", padding: "0 28px" }}>
      ...
      <div style={{ display: "flex", gap: "6px", marginTop: "10px", paddingBottom: "12px", overflowX: "auto" }}>
        {tabs.map((tab) => {
          ...
          const unlocked = isTabUnlocked ? isTabUnlocked(tab.id) : true;
          return (
...
function NoMigrantScreen({ onContinue }) {
  return (
    <div style={{ maxWidth: "860px", margin: "30px auto", padding: "0 24px" }}>
      <div style={{
        ...
        <GreenCheckCircle />
        <h2 style={{ margin: "10px 0 0", fontSize: "20px", fontWeight: "700", color: "#0F172A" }}>
          No Migrant Employees
        </h2>
...
function RTWVerificationScreen({ migrants, onBackToStaffList }) {
  ...
  return (
    <div style={{ maxWidth: "860px", margin: "30px auto", padding: "0 24px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px", flexWrap: "wrap" }}>
        ...
        >
          Employee {currentIndex + 1} of {migrants.length}
        </button>
      ...
      <div style={{
        ...
        <h3 style={{ margin: "0 0 5px", fontSize: "16px", fontWeight: "700", color: "#0F172A" }}>
          RTW Document Verification
        </h3>
        <p style={{ margin: "0 0 18px", fontSize: "13px", color: "#64748B" }}>
          Verify the uploaded RTW document and extracted information
        </p>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        {migrants.length > 1 && (
          <div style={{ display: "flex", gap: "10px" }}>
            {currentIndex > 0 && (
              <button onClick={() => setCurrentIndex(currentIndex - 1)} style={{ padding: "10px 18px", backgroundColor: "white", color: "#475569", border: "1.5px solid #E2E8F0", borderRadius: "8px", fontSize: "13.5px", fontWeight: "500", cursor: "pointer" }}>
                ← Previous
              </button>
            )}
            {currentIndex < migrants.length - 1 && (
              <button onClick={() => setCurrentIndex(currentIndex + 1)} style={{ padding: "10px 18px", backgroundColor: "#0852C9", color: "white", border: "none", borderRadius: "8px", fontSize: "13.5px", fontWeight: "600", cursor: "pointer" }}>
                Next Employee →
              </button>
            )}
          </div>
...
export default function RTWCompliance() {
  ...
  const handleTabClick = (tabId) => {
    ...
    if (!isTabUnlocked(tabId)) return; // locked
    const routes = {
  ...
  const handleContinueToPension = () => {
    markRTWComplete();
    router.push("/employer/sections/pension");
  };
