...
function clean(raw: any): string {
  ...
  if (typeof raw !== 'string') {
    console.warn('[clean] received non-string:', typeof raw, JSON.stringify(raw).slice(0, 50));
  ...
  if (clientToken) {
    ...
    if (typeof clientToken === "object") {
      ...
      if (typeof extracted === "string" && extracted.length > 10) {
        console.log("[resolveToken] extracted token from object");
        return extracted;
      }
    }
    ...
    if (typeof clientToken === "string") {
      const cleaned = clientToken
        .replace(/\s+/g, "")
        .replace(/^(Bearer|Token)\s*/i, "");

      if (cleaned && cleaned !== "undefined" && cleaned !== "null") {
        console.log("[resolveToken] using clientToken:", cleaned.slice(0,20)+"...");
        return cleaned;
      }
    }
  }
...
function statusToResult(s?: string) {
  return ({ pending: 'Pending', in_progress: 'In Review', approved: 'Approved', rejected: 'Rejected', completed: 'Completed' } as any)[s?.toLowerCase() ?? ''] ?? 'Pending';
}
...
export async function getAllTasksAction(
  ...
  const tasks: TaskItem[] = records.map(r => ({
    ...
    dateCreated: r.created_at ? new Date(r.created_at).toLocaleDateString('en-GB') : '—',
...
export async function getPostComplianceTasksAction(page = 1, clientToken?: string): Promise<AR<{ tasks: TaskItem[]; totalPages: number }>> {
  return { success: true, message: 'OK', data: { tasks: [], totalPages: 1 } };
}
export async function getCallAgentsTasksAction(page = 1, clientToken?: string): Promise<AR<{ tasks: TaskItem[]; totalPages: number }>> {
  return { success: true, message: 'OK', data: { tasks: [], totalPages: 1 } };
}