...
const getProgress = () => { try { return JSON.parse(sessionStorage.getItem("hr_progress") || "{}"); } catch { return {}; } };
const markComplete = (key) => { try { const p = getProgress(); sessionStorage.setItem("hr_progress", JSON.stringify({ ...p, [key]: true })); } catch {} };
...
function StepPills({ active, onStepClick, unlockedUpTo }) {
  return (
...
function RadioRow({ value, selected, onChange, label, desc }) {
...
function CashFlowStep({ onNext, onPrev, onSave }) {
  ...
  const handleNext = () => { onSave({ incoming: inNum, outgoing: outNum, netCashFlow: net }); onNext(); };
...
const COMPLIANT_REFS = ["business name","invoice number","client payment"];
...
function InvestmentsStep({ onNext, onPrev, onSave }) {
  ...
  const handleNext = () => { onSave({ transactions }); onNext(); };
...
function ContractsSyncStep({ onComplete, onPrev, savedContracts }) {
  const [paymentsReflected, setPaymentsReflected] = useState(null);
  const [futureEngagement, setFutureEngagement] = useState(null);
        ...
        <div style={{backgroundColor:"#F8FAFC", borderRadius:"8px", border:"1px solid #E2E8F0", padding:"14px 18px", marginBottom:"18px"}}>
          ...
          {savedContracts.map((c, i) => (
            <div key={i} style={{display:"flex", alignItems:"center", gap:"8px", marginBottom:"5px"}}>
              <FileIcon/>
              <span style={{fontSize:"13.5px", color:"#374151"}}>{c.clientName || c}</span>
            </div>
          ))}
        </div>
      )}
...
export default function FinancialPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (c) setSavedContracts(JSON.parse(c));
    } catch {}
  }, []);
  ...
  const handleTabClick = (tabId) => {
    ...
    const routes = { staff:"/employer/sections/hr-validation", rtw:"/employer/sections/hr-validation/rtw-compliance", pension:"/employer/sections/hr-validation/pension", auth:"/employer/sections/hr-validation/authorising-officer", contracts:"/employer/sections/hr-validation/contracts", summary:"/employer/sections/hr-validation/summary" };
  ...
  const handleSave = (data) => setFinancialData(prev => ({ ...prev, ...data }));
...
const inputStyle = { width:"100%", padding:"10px 12px", borderRadius:"8px", border:"1.5px solid #0852C9", fontSize:"14px", outline:"none", boxSizing:"border-box", color:"#0F172A", backgroundColor:"white" };
const primaryBtn = { padding:"13px 20px", borderRadius:"8px", border:"none", backgroundColor:"#0852C9", color:"white", fontSize:"14px", fontWeight:"600" };
const backBtn = { padding:"10px 20px", backgroundColor:"white", color:"#374151", border:"1.5px solid #D1D5DB", borderRadius:"8px", fontSize:"14px", fontWeight:"500", cursor:"pointer" };